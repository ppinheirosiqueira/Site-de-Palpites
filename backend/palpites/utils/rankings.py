...
from .score import check_diferenca_gols, check_pontuacao, check_pontuacao_grupo, check_diferenca_gols_grupo
...
def rankingGrupo(grupo):
    ...
    difGols = [check_diferenca_gols_grupo(palpites.filter(usuario=pessoa).exclude(partida__golsMandante=-1, partida__golsVisitante=-1), grupo) for pessoa in ids]
